...
function WheelMode({ onResult, accent, cardBg }) {
  ...
  const draw = useCallback((angle) => {
    ...
    names.forEach((name,i) => {
      ...
      ctx.textAlign="right";
      ctx.font=`bold ${names.length>14?11:13}px Cairo,sans-serif`;
      ctx.fillStyle="white"; ctx.shadowColor="#00000088"; ctx.shadowBlur=4;
    ...
    ctx.fillText("🎯",cx,cy);
  },[names,sliceAngle,accent,cardBg]);
  ...
  const spin = () => {
    ...
    const targetAngle = -(finalIdx*sliceAngle+sliceAngle/2)+Math.PI/2;
    const extraSpins = (4+Math.random()*4)*Math.PI*2;
    const totalAngle = extraSpins+targetAngle-(angleRef.current%(Math.PI*2));
    const duration = 3500+Math.random()*1000;
    const start = performance.now(), startAngle = angleRef.current;
    const animate = (now) => {
      const t = Math.min((now-start)/duration,1);
      const ease = 1-Math.pow(1-t,4);
      angleRef.current = startAngle+totalAngle*ease;
      draw(angleRef.current);
      if (t<1) { rafRef.current=requestAnimationFrame(animate); }
      else { setSpinning(false); setResult(finalName); onResult(finalName); }
    };
    ...
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:16}}>
      <div style={{position:"relative"}}>
        <div style={{
          ...
          borderRight:`26px solid ${accent}`,
          filter:`drop-shadow(0 0 8px ${accent})`,zIndex:10,
        }}/>
    ...
    [900,1300,1700].forEach((delay,ci)=>{
      setTimeout(()=>{
        ...
        if (ci===2) {
          ...
          setCols(prev=>{ const n=[...prev]; n[ci]=idx; return n; });
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          fontSize:"0.68rem",letterSpacing:3,color:`${theme.accent}99`,
          fontWeight:800,marginBottom:4,paddingRight:4,whiteSpace:"nowrap",
        }}>🎮 الأنماط</div>
        {MODES.map(m=>(
      ...
      <div style={{
        ...
        <div style={{textAlign:"center",marginBottom:24}}>
          ...
          <div style={{
            fontSize:"0.85rem",color:`${theme.accent}99`,
            letterSpacing:2,marginTop:4,fontWeight:600,
          }}>
